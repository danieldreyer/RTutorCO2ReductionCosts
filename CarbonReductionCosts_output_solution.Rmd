...
ggplot(pred_dat,aes(x=costratio,y=dat_east$co2mass))+
    geom_line(aes(x=costratio, y=co2.hat1), color="red") +
    geom_line(aes(x=costratio, y=co2.hat2), color="green") +
...
### Award: Linear Regression Expert2
TEXT
...
**Task:** Just press *check*.
```{r "4_1__40",warning=FALSE, fig.width=7, fig.height=7}
...
```{r "4_1__43",warning=FALSE, fig.width=14, fig.height=7}
...
**Task:** Just press *check*.
```{r "4_2__48",fig.width=7, fig.height=7}
...
Quiz: We have seen in an earlier exercise, that interconnection `ERCOT` has higher generation of renewable energies than `EAST`. Do you expect `ERCOT` to have a higher or lower emission abatement in comparison to `EAST`?
...
**Task:** Just press *check*.
```{r "4_2__51",fig.width=21, fig.height=7}
grid.arrange(plot_east, plot_ercot, plot_wecc, nrow=1)
```
