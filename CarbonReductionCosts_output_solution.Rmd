...
Quiz: What happens to our Cost Ratio in Equation 2, if Natural Gas doubles in price and Coal Prices goes down by 25%.
...
Quiz: Can we create any cost ratio under the assumption that we introduce carbon prices under fixed fuel prices (the answer is in the plots above)?
...
```{r "3_2__34",eval=FALSE}
predict.robust <- function(model, predict) {
  require(sandwich)
...
**Task:** Just press *check*.
```{r "4_1__46"}
# define base prices as mentioned in Exercise 2
basecoal=2.25
basegas=5.75
...
```{r "4_1__49",warning=FALSE, fig.width=7, fig.height=7}
...
```{r "4_1__50",warning=FALSE, fig.width=14, fig.height=7}
...
write.csv(dat_final, "X:\\libraries\\RTutor_BA\\exercise4_1.csv", row.names = FALSE)
```
...
```{r "4_2__53",message=FALSE}
#___ <- dat %>% 
#  mutate(carbontax = (costratio*___-___)/(___-costratio*___)) %>% 
#
#head(___,___)
dat_tax <- dat %>% 
  mutate(carbontax = (costratio*basegas-basecoal)/(co2_coal-costratio*co2_gas)) 
```
...
**Task:** Just press *check*.
```{r "4_2__57",fig.width=7, fig.height=7}
...
**Task:** Just press *check*.
```{r "4_2__58",fig.width=21, fig.height=7, warning=FALSE}
grid.arrange(plot_east, plot_ercot, plot_wecc, nrow=1)
```
...
Quiz: Assume we introduce a carbon tax of $20/ton, roughly how much would that cost us in billion dollars?
...
Continue we create a table which 
...
```{r "5__61",optional=TRUE, message=FALSE}
TODO
```
