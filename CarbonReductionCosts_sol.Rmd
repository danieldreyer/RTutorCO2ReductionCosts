...
libs = c("dplyr", "ggplot2", "gridExtra", "ggalt", "kableExtra", "RColorBrewer", "sandwich", "splines", "stargazer", "tidyverse", "scatterplot3d") # character vector of all packages you load in the problem set
...
#< quiz "Cost Ratios"
question: What happens to our Cost Ratio in Equation 2, if Natural Gas doubles in price and Coal Prices goes down by 25%.
sc:
    - Rise
    - Equal
    - Fall*
...
#< quiz "carbon Prices"
question: Can we create any cost ratio under the assumption that we introduce carbon prices under fixed fuel prices (the answer is in the plots above)?
sc:
    - Yes*
    - No
success: Great, your answer is correct!
...
#< award "Theorist"
Even though this chapter was a bit "dry", you learned quite a bit! You are ready to start working on the analysis.
#>
...
`Lm()` is part of the `stats` package, which is loaded on default. As shown in the syntax example below, it enables you us to regress `y` on the indepedent variables `x1` and `x2`. The model itself is handled just like every other variable, which can be either stored or used in other functions. Another popular function to solve linear regressions in `R` is `felm()`, that provides further functionality e.g. for including `fixed effects`. Because we won't model fixed effects here, we will strictly use `lm()`.
...
#< info "Predict"
We can predict the corresponding fitted values of our model with the `predict()` function. At minimum arguements it takes the fitted model and a data frame in which the functions looks for variables to predict from:  
...
Splines that we create with `ns()` are restricted by definition of the function. A similiar function that doesn't do that is `bs()`.
...
In this exercise we will finally bring together all our expertise and focus on our main results. In the first step we will perform the regression model based on the implications we made in `Exercise 3.2`. Afterwards we will use this model to predict emissions and trace our the emission response curves to changing fuel prices. We will explain and carry out the analysis step by step for interconnection `EAST`, afterwards apply it to `ERCOT` and `WECC` and interpret our results.  
...
$X_{t}$: Factors like non-fossil electricity production (e.g. solar, hydro or wind), $SO_{2}$ prices, net imports of electricity from Canada and variance in load  
$D_\gamma$: Dummy variable for seasonal variation to absorb fluctuations
#>

**Task:** Since we will run this regression model one times each for every interconnection, we can save some work and pre-define our model. Define the model according to `Equation 5` and store it as `model`.
```{r "4_1__5"}
#< task
...
We mentioned this before, but because of the fact that the spline coefficients are transformations of explanatory variables, we forego looking at the summary statistics. Instead we continue with our workflow. In the next step we predict the fitted values. To do this we follow the method of `Exercise 3.2` and use our custom funtion `predict.robust()`, which combines `predict()` and evaluation of`confidence intervals` from robust errors.  
...
At this point we are able to plot our results. As seen before we would visualize the relation between the absolute values of fitted $CO_2$ emissions and cost ratios. This was an elegant way to introduce our model, but our final results should give us an clearer indication. Since cost ratios can be constructed with a variety of different levels of coal and gas costs, it is unclear which specific prices of gas would imply certain changes in emissions. To avoid confusion we can simply transform our cost ratios backwards and only include decreasing costs of natural gas.  
Secondly, we will transform emission from a absolute scale to show the relative change in relation to a base line. Meaning, we show the relative change in emissions in realtiv to predicted fuel costs for 2025, as introduced in `Exercise 2`.  

**Task:** Below I implimented the changes from above a commented each step. We can tranform cost ratios directly in the process of creating our plot in the following code chunk. Just press *check*.  
```{r "4_1__8"}
...
Roughly comparing the results we get the same response curves for all interconnections. We can achieve reductions in $CO_2$ emissions of roughly 12% when introducing gas costs of $2 and we have increasing emissions for higher costs than our base line (we explained the reason above). Although by looking closely we can detect some differences by comparing the new graphs to the Eastern Interconection.  
For the Texas Interconnection (`ERCOT`) we observe a fall in emission that starts with lower gas costs and at the same time is steeper. If we look at the summary table of `Exercise 4` we can come up with some explanations. The proportion of non-fossil generated power is way lowere than for other regions. this will results in strong dependencies to the generators that are available at this moment. Fuel-switching will thereby occur later, but when the economic pressure is high enough, more strongly. We observe the same effect for increasing gas prices. The marginal costs for coal generators and other energy sources become more favorable with higher gas costs. Nevertheless the grid is kind of static, therefore the effects kicks in later but steeper.  
The respone curve of the `Western Interconnection` in contrast reacts much smoother to changing gas costs. Because of the higher proportion of non-fossil power generation of roughly 30%, the region is less effected by fuel-switching in the short term. The curve responds very static over a wider range of gas costs without any change in emissions. Additionally, for higher gas price the increase in emissions is clearly lower than for other regions. With falling gas price the grid has a longer response until emissions fall. All of these point are also supported by the fact, that the emission rate of the Western Interconnection is about 25% lower.  
  
Nevertheless, gas prices are set by supply and demand and without introducing very specific instruments to influence the industry, it's hard to directly set pricing as an government. In the next exercise we will solve this problem by tranforming our fuel costs into carbon prices and conclude our analysis. Click `Go to next exercise` to continue.

#< award "Sp(l)ine Surgeon2"
You sucessfully combined most of our introduced methods. Keep going to see the final results, the hardest part is over.
#>
...
#>
dat_tax <- dat %>% 
  mutate(carbontax = (costratio*basegas-basecoal)/(0.1054-costratio*0.0585))
```
...
As mentioned before, setting fuel prices directly is hardly possible. Therefore the plots above correspond to positive carbon prices within a reasonable range. In counterpart to our previous view on fuel costs, we lose some depth in the movement of the curves, but also are able to directly interpret the effects of an viable instrument in form of taxes.  
Since our method only transforms the x-Axis of our plots, the reaction curves look quite similiar and the therefore the reasons for different reactions are also the same. The central point here is to determine which levels of carbon prices would be most efficient. The Texas Interconnections reacts steeper for lower carbon prices, while the reaction of the Eastern und Western region react is more balanced.  
Based on the results of our plots, we can achieve clear reductions in emissions by introducing even smaller tax rates. If we only take the positive effects of reduced emissions on the environment into account, we would introduce the highest possible taxes. But since we are living in a economy where every additional charge can have severe consequences we have to carefully weigh our actions.  Based on the graphical interpretation, a tax between \$10 and \$40 seems to have the most impact on $CO_2$ emissions when considering the value per dollar.

&nbsp;
#< quiz "Costs of carbon taxes"
question: Assume we introduce a carbon tax of $50/ton, roughly how much would that cost us in billion dollars?
...
You see, even "moderate" tax rates can lead to extreme costs. Considering that the economic effect of carbon prices is controversal, we have to find a socially acceptable level. Next, we will construct a table, which gives us information on the effects of certain levels of taxes on emissions and at the same time calculates the corresponding costs.
