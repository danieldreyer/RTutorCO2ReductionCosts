...
```{r "3_1__15"}
# ... <- lm(...~... , data=east)
fit1 <- lm(co2mass ~ costratio, data=dat_east)
```
...
```{r "3_1__21"}
fit2 = lm(co2mass ~ costratio + load, data=dat_east)
...
Simply evaluating the fitted values on our intitial data frame won't give us an interpretable curve though. To illustrate this problem run the code below.

**Task:** Predict the fitted values of `fit2` on `dat_east` and plot the result. Just click **check**.
...
co2.hat2 = predict(fit2, dat_east)
```


A multivariate regression with two dependent variables spans three dimensions.



```{r "3_1__23"}
x <- dat_east$costratio
y <- co2.hat2
z <- dat_east$load

s3d <- scatterplot3d(x,z,y, col.axis = "blue",
angle = 55, scale.y = 0.7, pch = 16, main = "3D Regression Plane", xlab="costratio", ylab = "load", zlab = "emissions")
s3d$plane3d(fit2, lty = "dotted")
```


```{r "3_1__24"}
ggplot(dat_east,aes(x=costratio,y=co2.hat2))+
        geom_line(aes(x=dat_east$costratio, y=co2.hat2))
```



The problem hereby lies in 


We intend to plot our results in terms of reponse curves. To be able to do so we won't predict our values on the original data frame we build our model on. Instead we will use the implication that the average predicted values from a regression model will equal the average of the dependet variables. In mathematical terms we can explain this as followed:
A linear regression model with intercept term satifies following solution $X^tX\beta=X^ty$
$$\sum_i\sum_jx_{ij}\beta_j=\sum_iy_i$$
...
fit2 = lm(co2mass ~ costratio + load, data=dat_east)
fit3 = lm(co2mass ~ poly(costratio,3) + load, data=dat_east)
```

```{r "3_1__26"}
show.regression(fit1, fit2, fit3)
...
```{r "3_1__28"}
plot(pred_dat$costratio, dat_east$co2mass, ylim=range(CO2.hat1, CO2.hat2))
lines(pred_dat$costratio, CO2.hat1, col="black")
lines(pred_dat$costratio, CO2.hat2, col="green")
lines(pred_dat$costratio, co2.hat3, col="red")


ggplot(pred_dat,aes(x=costratio,y=co2mass))+
        geom_point(alpha=0.2) +
        geom_line(aes(x=pred_dat$costratio, y=CO2.hat1)) +
  geom_line(aes(x=pred_dat$costratio, y=CO2.hat2)) +
  geom_line(aes(x=pred_dat$costratio, y=co2.hat3))
