...
The pipe operator `%>%` is a feature provided by the `dplyr` Package. Essentially it allows to exectute multiple operation on a dataframe at once. Essentially, every pipe operator returns a dataframe and passes it to the next connected function.  
`Select` allows you to filter for certain columns of data, whereas `filter` does the same for rows.  
...
`lm()` is part of the `stats` package and is part of basic `R`. As shown in the syntax example below, it enables you to regress `y` on the indepedent variables `x1` and `x2`. The model itself can be handled just like every other variable. You can give it a name or use it in other functions. Another popular function to solve linear regressions in `R` is `felm()`, that provides further functionality to include `fixed effects`. In this problem set we will strictly use `lm()`.
...
`R` provides an easy framework to add your own functions. The syntax is quite similar to other programming languages you could be familiar with. As long as you are in the same session you can then use this function. Below you find the basic structure and an example:

```{r "3_1__16",eval=FALSE}
...
### Info: R-squared
`R-Squared` is a statistical measurement that represents the correlation between `fitted values` and `observed values`. `R-squared` is hereby always positive and ranges from 0 to 1. A value closer to 1 indicates that the suggested model explains a majority of the variance in the outcome variable.  Mathematically we can descibe this with the following: 
...
**Task:** Just press **check**
```{r "3_1__23",fig.width=9, fig.height=6, dev='svg'}
...
### Info: ns()
`ns()` is part of the `splines` package and allows us to perform natural splines regressions on our data. It takes several input arguments, the only one we will use is `df` which sets the degrees of freedom. `Df` hereby define the degrees of freedom and the function chooses df-1-intercept knots, which are set at predefined quantiles.  
In our case we will choose `df=5`, which places our knots at following quantiles: `[0.05, 0.275, 0.5, 0.725, 0.95]` (Harrell, 2001).

A simple syntax example of cubic spline regression: 

```{r "3_2__30",eval=FALSE}
...
The summary statistics of our spline model is comparable to the one of simple polynomial regressions, we get a coefficient for every knot. Since splines are transformations of explanatory variables it isn't really possible to interpret these coefficients straightforward. Because of that reason we went through the trouble of introducing the method of plotting regression models. 

### Newey-West Standard Error

We continue to see a negative relationship between cost ratios and $CO_2$ emissions, but additionally plotted the data points of emissions. As cost ratios increase, the difference between our regression line and the observations tend to become larger. This is a strong indicator that our model suffers from `heteroskedasticity`. Since standard error methods rely on the assumption that theres no correlation between the independent variables and the variance of the dependent variables, we calculate `heteroskedasticity robust standard errors`.  
R packages normally provide a lot of handy functions to predict fitted values and calculate robust standard errorsat the same time. However, we don't predict our fitted values on the same data frame as we build our model on. Therefore this will be a bit tricky. We will use a custom function that is based on the Newey-West estimator `NeweyWest()`, which is part of the `sandwich` package. Therefore, we will be able to calculate the error of the upper and lower bounds and use these later on to add confidence intervals to our plot. 
////////
...
Before continuing with our main analysis we have to introduce the method of calculating the errors of our model. If you are already familiar with how standard error you should know that we wont just use the standard errors. We can easily show that by plotting `fit3` with the methods we used before: 

**Task:** Just press **check**.
```{r "3_2__34",warning=FALSE}
...
**Task:** Just press *check*.
```{r "4_1__46",warning=FALSE, fig.width=7, fig.height=7}
...
```{r "4_1__47",warning=FALSE, fig.width=7, fig.height=7}
...
```{r "4_1__48",warning=FALSE, fig.width=7, fig.height=7}
...
```{r "4_1__49",warning=FALSE, fig.width=14, fig.height=7}
...
**Task:** Just press *check*.
```{r "4_2__54",fig.width=7, fig.height=7}
...
**Task:** Run the code for the `ERCOT` interconnection. Just press *check*.
```{r "4_2__55",fig.width=7, fig.height=7}
...
**Task:** Run the code for the `WECC` interconnection. Just press *check*.
```{r "4_2__56",fig.width=7, fig.height=7}
...
**Task:** Just press *check*.
```{r "4_2__57",fig.width=21, fig.height=7}
grid.arrange(plot_east, plot_ercot, plot_wecc, nrow=1)
```
