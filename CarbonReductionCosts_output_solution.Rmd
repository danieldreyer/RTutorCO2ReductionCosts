...
The world has to further reduce it's $CO_2$ emissions in order to meet the requirements of the Paris Convention on Climate Neutrality by 2050. Right now the energy consumption keeps rising daily and even though new technologies are being developed to increase the percentage of `green` electricity, we are still heavily dependant on `fossil fuels`. `Coal` is hereby one of the most important sources of energy, but at the same time the biggest cause of carbon emissions. Reasons being, that it is easy to store, transport, doesn't alter and is mined in large quantities around the world. Given that coal supplies will last for centuries, it is a major goal to find incentitives to reduce it's use and therefore $CO_2$ `emissions`. One solution would be to improve the efficiency of coal-generators, another one to replace it with another fuel type.
One alternative is `natural gas`, which is obviously also a type of fossil fuel, but it's $CO_2$ emissions and heat density are far more efficient. Nevertheless, natural gas wasn't considered to be an real alternative to coal because of higher costs through more difficult extracting methods and handling.  
...
Quiz: Did the proportion of gas-generated electicity has increased or decreased over time in comparison to coal-generated electricity ?
...
In this exercise we convert the mechanism of `fuel switching` from last exercise into a theory that we can use for our analysis. Our obvious goal is to find a connection between emissions and fuel costs to use as a basis for a model. We already introduced factors that influence the cost efficiency of certain fuel-generators as well as 


We already mentioned that the usage of certain generators isn't purely limited by cost efficiency. If we assume that we aren't limited by , `Marginal Costs` will play a big role in deciding what fuel-generator we use for power generation. Marginal costs are per definition not only the fuel prices itself, but include all costs that are needed to produce the additional MWh of power. 
...
We have already seen in the previous exercise that the proportion of gas and coal have converged considerably over time. Because of the fact, that coal contains approximately twice as much $CO_2$ as natural gas, we can create further incentives to reduce emissions by introducing carbon prices. 
...
The idea of constructing the cost ratio of both fuels will allow us later to impelement fuel switching in our model. Because of it's importance, let's interpret `Equation 2`
You may be wondering why we would construct a cost ratio instead of using 



Quiz: What happens to our cost ratio, if natural gas doubles in price and coal prices go down 25%.

- cost ratio doubles [x]

- blabla [ ]

&nbsp;

Of course this relation is quite intuitive. By adding carbon prices as in Equation 3 the relation gets a bit more complex. For a clearer picture, lets look at the visualization below. The values are based on prices and carbon content from above:
...
Quiz: Based on the graph above, can we create any cost ratio under the assumption that we introduce carbon prices under fixed fuel prices?
...
**Task:** Since we will run this model for every interconnection we can first define our model and save it to a variable. Afterwards we can run it on each data frame. Define the model according to `Equation 5` and save it to `model`.
```{r "4_1__42"}
#___ co2mass ~ ___(priceratio, df=...) + ___(load, df=...) + tlsd + tlmin + tlmax + ___(meant, df=...) + nonfossil + so2price + netNSflow + yearseason
model=co2mass ~ ns(costratio, df=5) + ns(load, df=5) + tlsd + tlmin + tlmax + ns(meant, df=5) + nonfossil + so2price + netNSflow + yearseason
```

**Task:** Run the regression with `lm()`, our model `formula` and use the data frame `EAST`.
```{r "4_1__43"}
